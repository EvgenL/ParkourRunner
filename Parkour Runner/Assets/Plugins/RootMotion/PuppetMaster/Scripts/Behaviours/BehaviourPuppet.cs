		...
		protected override void OnFixedUpdate() {
			...
			if (state != State.Unpinned) {
				...
				foreach (Muscle m in puppetMaster.muscles) {
					...
					float offset = unpinnedMuscleKnockout? m.positionOffset.sqrMagnitude: m.positionOffset.sqrMagnitude * m.props.pinWeight;
					...
					if (hasCollidedSinceGetUp && !puppetMaster.isBlending && offset > 0f && m.state.pinWeightMlp < pinWeightThreshold && offset > props.knockOutDistance * knockOutDistanceSqr * stateMlp) {
