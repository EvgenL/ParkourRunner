...
namespace Basic_Locomotion.Scripts.HeadTrack.Scripts
{
    public class vHeadTrack : MonoBehaviour
    {
        ...
        bool lookConditions { get { return head != null && (followCamera && UnityEngine.Camera.main != null) || (!followCamera && (currentLookTarget || simpleTarget))|| temporaryLookTime>0; } }
        ...
        Vector3 GetLookPoint()
        {
            ...
            if (lookConditions && !IgnoreHeadTrack() )
            {
                ...
                if(temporaryLookTime<=0)
                {
                    ...
                    if (followCamera)
                    {
                        lookPosition = (UnityEngine.Camera.main.transform.position + (UnityEngine.Camera.main.transform.forward * distanceToLoock));
                    }
