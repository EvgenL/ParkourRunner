...
namespace Melee_Combat.Scripts.MeleeWeapon
{
    public class vMeleeWeapon : vMeleeAttackObject, vIEquipment
    {
        ...
        {
            var damage = item.attributes.Find(attribute => attribute.name ==  vItemAttributes.Damage);
            var staminaCost = item.attributes.Find(attribute => attribute.name == vItemAttributes.StaminaCost);
            var defenseRate = item.attributes.Find(attribute => attribute.name ==  vItemAttributes.DefenseRate);
            var defenseRange = item.attributes.Find(attribute => attribute.name == vItemAttributes.DefenseRange);
