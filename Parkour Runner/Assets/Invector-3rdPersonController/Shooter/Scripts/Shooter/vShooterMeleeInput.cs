...
using Shooter.Scripts.Generic;
using Shooter.Scripts.SimpleIK;
...
namespace Shooter.Scripts.Shooter
{
    ...
    public class vShooterMeleeInput : vMeleeCombatInput
    {
        ...
        void OnDrawGizmos()
        {
            ...
            var _ray = new Ray(aimAngleReference.transform.position + aimAngleReference.transform.up * shooterManager.blockAimOffset, aimPosition - (aimAngleReference.transform.position));
        ...
        protected virtual void RotateRightArm(bool isUsingLeftHand = false)
        {
            if (!shooterManager) return;
        ...
        protected virtual void RotateRightHand(bool isUsingLeftHand = false)
        {
            if (!shooterManager) return;
            var weapon = !isUsingLeftHand ? shooterManager.rWeapon : shooterManager.lWeapon;
