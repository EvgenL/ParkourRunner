...
namespace Basic_Locomotion.Scripts.Ragdoll
{
    ...
    public class vRagdoll : vMonoBehaviour
    {
        ...
        void OnDestroy()
        {
            ...
            {
                if (vGameController.instance && vGameController.instance.destroyBodyAfterDead)
                    Destroy(characterHips.gameObject);
                else
                    characterHips.SetParent(hipsParent);
            }
            if ((vGameController.instance && !vGameController.instance.destroyBodyAfterDead || vGameController.instance == null) && removePhysicsAfterDie)
            {
                DestroyComponents();
            }
        }
        ...
        private void ApplyVelocityToRagdoll(Vector3 velocity)
        {
            ...
            foreach (Component c in components)
            {
                if (!ignoreTags.Contains(c.transform.tag))
                    (c as Rigidbody).velocity = velocity;
            }
        }
