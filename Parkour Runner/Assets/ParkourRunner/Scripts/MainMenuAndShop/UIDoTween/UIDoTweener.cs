...
public class UIDoTweener : MonoBehaviour {
    ...
    private void BackToMain()
    {
       ...
       // firstSecuance.Insert(0.3f, _settingsPanelEndPos.DOAnchorPos(new Vector2(_settingsPanelEndPos.anchoredPosition.x, _settingsPanelStartPos.y), _settingsPanelDuration).SetEase(Ease.InOutElastic));
        ...
        firstSecuance.OnComplete(() => 
        {
            ...
            {
                StartCoroutine(EnableBtnsRaycastTargetTo(true, 0.1f));
                _shop.transform.GetChild(0).GetChild(1).gameObject.SetActive(true);
                AddListeners();
            });
        });
    }
